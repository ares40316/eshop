...
/**
 * Product（商品）實體類別 對應資料表：products 用於儲存商城商品的基本資訊，例如名稱、價格、庫存、上下架狀態與分類等
 */
@Entity
@Table(name = "products")
public class Product {
	...
	/**
	 * 商品所屬分類（一對多關聯到 ProductCategory 中介表） 一個商品可對應多個分類（多對多結構）
	 */
	@OneToMany(mappedBy = "product")
	private Set<ProductCategory> categories;

	// 一对多关联（重点注解配置）
	@OneToMany(mappedBy = "product", cascade = CascadeType.ALL, orphanRemoval = true, // 自动删除无关联的子项
	...
	public List<ProductImage> getImages() {
	    return images;  // 移除 unmodifiableList 包裝
	}
	...
	// 批量替換方法
	public void setImages(List<ProductImage> images) {  // 參數改為 List
	    this.images.clear();
