...
@Controller
public class ProductAction extends ActionSupport implements SessionAware , ServletRequestAware{
	...
	public String execute() {
		...
		if (categoryIds == null)
			categoryIds = java.util.Collections.emptyList();
		if (pageNo < 1)
			pageNo = 1;
		...
		try {
			ProductPageResult result = productService.searchWithFilterAndPaging(keyword, categoryIds, pageNo, 10);
			...
			for (Product p : productList) {
				List<String> urls = p.getImages().stream().map(img -> img.getImageUrl()).collect(Collectors.toList());
	...
	public String detail() {
	    try {
	        ...
	        if (p == null) {
	            addActionError("找不到這個商品");
	            return ERROR;
	        }
	        detailProduct = p;

	        // 正确从 ServletActionContext 拿 request
	        String xhr = ServletActionContext.getRequest()
                    .getHeader("X-Requested-With");
    boolean isAjax = "XMLHttpRequest".equals(xhr);
	        return isAjax ? "ajax" : SUCCESS;
	    } catch (Exception e) {
	        logger.error("Error loading product detail", e);
	        addActionError("載入商品詳情失敗: " + e.getMessage());
	        return ERROR;
	    }
	}
